...
In CFML, every time you call a method you won't get a value back. By default, a CFML method returns nothing. If you called the @make_toast@ method above, you should have seen the return value @nil@. The @cfreturn@ instruction returns a @value@, so since that wasn't the last instruction before the ending @cffunction@ in your method you saw @nil@ when calling that method.
...
*  @Replace@ 
The @Replace@ function replaces occurrences of substring1 in a string with substring2, in a specified scope. The search is case sensitive. For instance, @Replace("Hello", "e", "")@ would give you back @"hllo"@ after replacing the first occurrence of @"e"@, or @Replace("Good Morning!", "o", "e", "All")"@ would give you @"Geed Merning!"@ 
*  @Remove@ 
Call @remove@ to remove characters from a string.. For instance, @RemoveChars("hello bob", 2, 5)@ would give you back @"hbob"@.  
*  @Mid@ 
...
In the first line we setup a variable to hold the day of the week, then in the second line we printed the string @Happy@ combined with the value of the variable @today@ and the string @!@. You might be thinkingWhat was the point of that since we still just wrote Saturday in the first line?Ok, well, if you were writing a real program you'd use CFMLs built-in date functions like this:
...
today = Date.today.strftime("%A") day_of_year = Date.today.yday puts "Happy " + today + "! It is the " + day_of_year + " day of the year."

You should get an error complaining that CFML can't convert Fixnum into String. What does that mean? 
