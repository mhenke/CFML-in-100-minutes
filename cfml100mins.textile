...
TODO: CFML verses ColdFusion

ColdFusion was released and it grew an audience quickly in the government and public sector. CFML tag syntax resembles HTML and the script syntax resembles JavaScript. You may want to focus on either the tag or script based examples depending on your comfort level.
...
There are two ways to write CFML code. You can use the tag or script syntax. For the examples, please focus on one or the other so this tutorial is not confusing. You will write one or more instructions in a file then run that file through the CFML engine. 

h4. CFSET & CFOUTPUT

This will mention CFSET AND CFOUTPUT

We might have a file named my_program.cfm and Sample.cfc like this:
...
For an example of an object, think about you as a human being. You have attributes like height, weight, and eye color. You have methods like walk, run, wash dishes, and daydream. Different kinds of objects have different attributes and methods. In the next sections we'll look at a few specific kinds of objects common in CFML.
...
To return data, we use @cfreturn@ to instruct the function to return a @value@. Since that wasn't the last instruction before the ending @cffunction@ in your @make_toast@ method, you received @null@ and tried to putting that into the @result@ variable.
