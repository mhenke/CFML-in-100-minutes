...
ColdFusion did not have a way of referring to nothingness until version 9. ColdFusion could recieve a @NULL@ value from an external source and would maintain the @NULL@ value until you tried to use it. ColdFusion would convert the @NULL@ into an empty string (in the case of queries) or potentially destroy the variable altogether. However now with greater support for @NULL@ values, ColdFusion allows you to return a @NULL@ value from a function. @IsNull()@ instruction will test for @NULL@ values and return @true@ or @false@.
