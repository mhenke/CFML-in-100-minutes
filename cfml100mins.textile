...
CFML is a great language for beginners because it was written to make the programmer's job easy and not care if the computer's job is hard. In this brief introduction we'll look at the key language features you need to get started.
...
ColdFusion was released and it grew an audience quickly in the government and public sector. CFML tag syntax resembles HTML and its script syntax resembles JavaScript. You may want to focus on either the tag or script based examples depending on your comfort level.
...
In CFML, a ColdFusion component (CFC) file contains data and functions. Components are a building blocks for objects. Objects know information, called attributes, and they can do actions, called methods. In ColdFusion the @cffunction@ tag is used to define methods within a CFC.
...
In CFML we define an object using the @cfcomponent@ keyword and save the file as @.cfc@, here’s an example defining the object type @PersonalChef.cfc@:
...
In CFML, every time you call a method you won't necessarily get a value back. By default, a CFML method returns @null@ which ColdFusion can't handle. We'll talk about @null@ in the last section of CFML in 100 minutes. If you called @make_toast@ method above like @<cfset result = frank.make_toast('burned') />@ or @ set result = frank.make_toast('burned');@, and tried to output @result@ you should have seen @Variable RESULT is undefined.@.

@cfreturn@ instructs the function to return a @value@. Since that wasn't the last instruction before the ending @cffunction@ in your @make_toast@ method, you received @undefiend@ putting the calling of method into a variable.
...
<pre>
<code>
 today = Date.today.strftime("%A") day_of_year = Date.today.yday 
...
<code>
 def make_eggs(quantity) 
  if quantity.nil? 
   puts "How am I supposed to make nothingness number of eggs?" 
  else 
