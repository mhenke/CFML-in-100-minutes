...
In the first line we setup a variable to hold the day of the week. Then we'll printed the string @Happy@ combined with the value of the variable @today@ and the string @!@. You might be thinking. What was the point of that since we still just wrote Saturday in the first line? Ok, well, if you were writing a real program you'd use CFMLs built-in date functions like this:
...
Great, no errors and our output looks correct. There is another combination method called _string interpolation_.
