...
h3. An Aside on Running CFML from a File
...
def make_eggs(quantity) puts "Making you #{quantity} eggs!" return self end

Save the file in your editor. Then we need to tell CFML to re-read the file. We can do that with this instruction in %(caps)IRB% :
