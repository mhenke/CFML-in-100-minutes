...
There are two ways to write CFML code. You can use the tag or script syntax. For the examples, please focus on one or the other so this tutorial is not confusing. CFML includes a set of instructions that you use in pages.  You will write one or more instructions in a file then run that file through the CFML engine. Two ColdFusion instructions we will use in this tutorial are @CFSET@ and @CFOUTPUT@. @CFSET@ is used to create a variable and assign it a value. Also used to call functions. @CFCOUTPUT@ displays the variable's value. 
...
TODO: NEED TO CLAIRFY THIS SENTENCE Where the method is expecting us to pass in a @color@ telling it how to do the method @make_toast@.
...
To return data, we use @cfreturn@ to instruct the function to return a @value@. Since that wasn't the last instruction before the ending @cffunction@ in your @make_toast@ method, you received __nothing__ and tried to putting that into the @result@ variable.
...
*  @Replace@ 
The @Replace@ function replaces occurrences of substring1 in a string with substring2, in a specified scope. The search is case sensitive and the default scope is one. For instance, @Replace("Hello", "e", "")@ would give you back @"hllo"@ after replacing the first occurrence of @"e"@, or @Replace("Good Morning!", "o", "e", "All")"@ would give you @"Geed Merning!"@ 
*  @Remove@ 
...
Often with strings we want to pull out just a part of the whole this is called a substring. Try out these examples in the CFML file assuming from the last examples:
