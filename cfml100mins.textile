...
h3. New Operator in ColdFusion 9

In ColdFusion 9 you can also use the @New@ operator to create a new instance of an object and call it's @init()@ method (if it exists). Any arguments supplied to the object using the @New@ operator will be passed as arguments to the @init()@ method of the component. The @init()@ method should return the object instance using @return this;@ in order to have the same expected behavior as the @CreateObject@ statement. If no @init()@ method exists, no errors will be thrown, and the object will be returned normally:
...
Prior to ColdFusion 9, ColdFusion did not have a way of referring to nothingness. However, ColdFusion could recieve a @NULL@ value from an external source. ColdFusion would maintain the @NULL@ value until you tried to use it, and then ColdFusion would convert the @NULL@ into an empty string (in the case of queries) or potentially destroy the variable altogether. ColdFusion 9, however, has added greater support for @NULL@ values, allowing you to return a @NULL@ value from a function and use the @IsNull()@ function to test for @NULL@ values.
